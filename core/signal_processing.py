...
from .material_models import detect_end_index
...
def preprocess_data(data, diameter, height, strain_increment=0.005):
    ...
    if not np.isfinite(dx) or dx <= 0: raise ValueError("strain_increment must be a positive finite number.")

    def _grid_by_dx(x_max: float, dx: float) -> np.ndarray:
        xs = np.arange(0.0, x_max + 0.5 * dx, dx)
        if xs.size < 2: raise ValueError("strain_increment too large for the available strain range.")
        if xs[-1] > x_max + 1e-12: xs[-1] = x_max
        elif xs[-1] < x_max - 1e-12: xs = np.append(xs, x_max)
        return xs
    ...
    if not np.any(valid): raise ValueError("No valid Jaw_corr samples found.")
    ...
    data["y"] = ((data["Force"] * 1000.0) / ((math.pi / 4.0) * diameter**2 * (height / denom)))
    ...
    if cut is not None and cut > 10:
        x_raw = x_raw[:cut]
        y_raw = y_raw[:cut]
        if temp_raw is not None: temp_raw = temp_raw[:cut]
    ...
    x_raw = x_raw[keep]
    y_raw = y_raw[keep]
    if temp_raw is not None: temp_raw = temp_raw[keep]

    if x_raw.size < 5: raise ValueError("Not enough valid samples after trimming.")
        
    x_stress = x_raw
    y_stress = y_raw

    max_x_stress = float(np.max(x_stress))
    if not np.isfinite(max_x_stress) or max_x_stress <= 0: raise ValueError("Invalid strain range after preprocessing.")

    new_x = _grid_by_dx(max_x_stress, dx)
    
    fY = interp.interp1d(x_stress, y_stress, bounds_error=False, fill_value=(y_stress[0], y_stress[-1]))
    y_none = fY(new_x)
    ...
    if temp_raw is not None:
        fT = interp.interp1d(x_raw, temp_raw, bounds_error=False, fill_value=(float(temp_raw[0]), float(temp_raw[-1])))
